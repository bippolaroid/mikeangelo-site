...
export default function AdminNav() {
  ...
  createEffect(() => {
    ...
    if (prevScrollY() > 0) {
      ...
      navStyle!.style.backgroundColor = "#fafafa";
    } else {
  ...
  return (
    <>
      <nav id="nav" class="z-1000 fixed w-full flex xl:py-6 bg-transparent transition-all duration-500 border-b border-transparent">
        <div class="flex w-full max-w-7xl mx-auto p-4 xl:p-0 justify-between items-center">
          <div class="w-full flex justify-start">
            <A
              href="/"
              class="text-neutral-950 hover:text-neutral-700 cursor-pointer uppercase tracking-widest"
            >
              Mike Angelo
            </A>
          </div>
          {/*<Show when={!isMobile()}>*/}
            <div class="w-full flex gap-4 justify-end items-center">
              <A href="">
                <button class="bg-neutral-950 hover:bg-neutral-700 px-4 py-2 cursor-pointer rounded text-neutral-50">
                  Settings
                </button>
              </A>
            </div>
      ...
      <div class="h-[3.5rem] lg:h-[4.5rem] xl:h-[5.5rem]"></div>
    </>
  );
}
